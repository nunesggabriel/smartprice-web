...
const Produtos = () => {
  ...
  const handleSalvar = async () => {
    ...
    if (!form.referencia) {
      setErro('A referência é obrigatória.');
      return;
    }

    if (!form.producao_mensal || Number(form.producao_mensal) <= 0) {
      setErro('Informe a produção mensal.');
      return;
    }
  ...
  const handleVincularInsumo = async () => {
    ...
    if (!formInsumo.insumo_id) {
      setErro('Selecione um insumo.');
      return;
    }

    if (!formInsumo.quantidade || Number(formInsumo.quantidade) <= 0) {
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Gerencie os produtos da sua confecção</p>
        </div>
        <button
      ...
      {erro && !modalAberto && !modalInsumoAberto && (
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                <th className="text-left text-xs font-medium text-gray-500 px-6 py-3">Referência</th>
                <th className="text-left text-xs font-medium text-gray-500 px-6 py-3">Descrição</th>
                <th className="text-left text-xs font-medium text-gray-500 px-6 py-3">Produção mensal</th>
                <th className="text-left text-xs font-medium text-gray-500 px-6 py-3">Custo base</th>
            ...
            <tbody>
              {produtos.map((produto, index) => (
                <tr key={produto.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  <td className="px-6 py-4 text-sm font-medium text-gray-800">{produto.referencia}</td>
                  <td className="px-6 py-4 text-sm text-gray-600">{produto.descricao ?? '—'}</td>
                  <td className="px-6 py-4 text-sm text-gray-600">{produto.producao_mensal ?? '—'} un/mês</td>
                  <td className="px-6 py-4 text-sm text-gray-600">
                    {produto.custo_base ? `R$ ${Number(produto.custo_base).toFixed(2)}` : '—'}
                  </td>
                  <td className="px-6 py-4 text-sm flex gap-3">
                    <button
                      onClick={() => abrirModalInsumos(produto)}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Ex: Camiseta básica manga curta"
                  className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Produção mensal (unidades) *</label>
                <input
                  type="number"
                  value={form.producao_mensal}
                  onChange={(e) => setForm({ ...form, producao_mensal: e.target.value })}
                  placeholder="Ex: 100"
                  className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl shadow-lg w-full max-w-lg p-6">
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-lg font-bold text-gray-800">Insumos do produto</h3>
                <p className="text-sm text-gray-500">{produtoSelecionado.referencia} — {produtoSelecionado.descricao}</p>
              </div>
              <button
                onClick={fecharModalInsumo}
            ...
            <div className="mb-4">
              {insumosVinculados.length === 0 ? (
                ...
                <table className="w-full mb-2">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200">
                      <th className="text-left text-xs font-medium text-gray-500 px-4 py-2">Insumo</th>
                      <th className="text-left text-xs font-medium text-gray-500 px-4 py-2">Qtd</th>
                      <th className="text-left text-xs font-medium text-gray-500 px-4 py-2">Custo unit.</th>
                      <th className="text-left text-xs font-medium text-gray-500 px-4 py-2">Total</th>
                      <th className="px-4 py-2"></th>
                    </tr>
                  </thead>
                  <tbody>
                    {insumosVinculados.map((pi: any, index: number) => (
                      <tr key={pi.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                        <td className="px-4 py-2 text-sm text-gray-800">{pi.nome}</td>
                        <td className="px-4 py-2 text-sm text-gray-600">{pi.quantidade}</td>
                        <td className="px-4 py-2 text-sm text-gray-600">R$ {Number(pi.custo_unitario).toFixed(2)}</td>
                        <td className="px-4 py-2 text-sm font-medium text-gray-800">R$ {Number(pi.custo_total_linha).toFixed(2)}</td>
                        <td className="px-4 py-2">
                          <button
                            onClick={() => handleRemoverVinculo(pi.id)}
            ...
            {/* Formulário para vincular novo insumo */}
            <div className="border-t border-gray-200 pt-4">
              <p className="text-sm font-medium text-gray-700 mb-3">Adicionar insumo</p>
              <div className="flex gap-2">
                <select
                  value={formInsumo.insumo_id}
                  onChange={(e) => setFormInsumo({ ...formInsumo, insumo_id: e.target.value })}
                ...
                />
                <button
                  onClick={handleVincularInsumo}
                  className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 rounded-lg transition-colors"
                >
                  Adicionar
                </button>
              </div>
