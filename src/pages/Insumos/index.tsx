...
const Insumos = () => {
  ...
  const handleSalvar = async () => {
    ...
    if (!form.custo_unitario || Number(form.custo_unitario) <= 0) {
      setErro('Informe o custo unitário.');
      return;
    }
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Cadastre a matéria prima utilizada nos produtos</p>
        </div>
        <button
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="text-left text-xs font-medium text-gray-500 px-6 py-3">Custo unitário</th>
                <th className="text-left text-xs font-medium text-gray-500 px-6 py-3">Ações</th>
              </tr>
            </thead>
            <tbody>
              {insumos.map((insumo, index) => (
                <tr key={insumo.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  <td className="px-6 py-4 text-sm font-medium text-gray-800">{insumo.nome}</td>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl shadow-lg w-full max-w-md p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Ex: Tecido malha fria"
            ...
            <div className="flex gap-3 mt-6">
              ...
              >
                {salvando ? 'Salvando...' : insumoEditando ? 'Salvar alterações' : 'Criar insumo'}
              </button>
            </div>
