...
const Despesas = () => {
  ...
  const abrirModalNovo = () => {
    ...
    setForm({ descricao: '', valor: '', data_vencimento: '', data_pagamento: '', status: 'pendente' });
    setErro('');
    setModalAberto(true);
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={abrirModalNovo} className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2.5 rounded-lg transition-colors">+ Nova despesa</button>
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden">
        {carregando ? (
          ...
          <div className="text-center py-12 text-gray-400 text-sm">Nenhuma despesa cadastrada. Clique em "+ Nova despesa" para começar.</div>
        ) : (
            ...
            <tbody>
              {contas.map((conta, index) => (
                <tr key={conta.id} className={index % 2 === 0 ? 'bg-white dark:bg-gray-900' : 'bg-gray-50 dark:bg-gray-800'}>
                  ...
                  <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">R$ {Number(conta.valor).toFixed(2)}</td>
                  <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">{conta.data_vencimento ? new Date(conta.data_vencimento + 'T00:00:00').toLocaleDateString('pt-BR') : '—'}</td>
                  <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">{conta.data_pagamento ? new Date(conta.data_pagamento + 'T00:00:00').toLocaleDateString('pt-BR') : '—'}</td>
                  <td className="px-6 py-4 text-sm"><span className={`px-2.5 py-1 rounded-full text-xs font-medium ${badgeStatus(conta.status)}`}>{conta.status ?? 'pendente'}</span></td>
      ...
      {modalAberto && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-lg w-full max-w-md p-6">
            <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100 mb-4">{contaEditando ? 'Editar despesa' : 'Nova despesa'}</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Descrição *</label>
                <input type="text" value={form.descricao} onChange={(e) => setForm({ ...form, descricao: e.target.value })} placeholder="Ex: Aluguel do galpão" className={inputClass} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Valor (R$) *</label>
                <input type="number" value={form.valor} onChange={(e) => setForm({ ...form, valor: e.target.value })} placeholder="Ex: 1500.00" step="0.01" className={inputClass} />
              </div>
              <div>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleSalvar} disabled={salvando} className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 text-white text-sm font-medium rounded-lg py-2.5 transition-colors">{salvando ? 'Salvando...' : contaEditando ? 'Salvar alterações' : 'Criar despesa'}</button>
            </div>
          </div>
        </div>
      )}
