...
const Configuracao = () => {
  ...
  const handleSalvar = async () => {
    ...
    if (!form.lucro_percentual || Number(form.lucro_percentual) <= 0) {
      setErro('Informe um markup válido.');
      return;
    }
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-0.5">Defina o markup padrão para a precificação</p>
      </div>
      ...
      {carregando ? (
        ...
        <div className="grid grid-cols-1 gap-6 max-w-2xl">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-6">
            <h3 className="text-base font-bold text-gray-800 mb-1">Markup padrão</h3>
            <p className="text-sm text-gray-500 mb-4">
              O markup é o multiplicador aplicado sobre o custo total do produto para definir o preço de venda.
              Um markup de 2.5 significa que o preço de venda será 2.5× o custo.
            </p>

            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Coeficiente de markup *
                </label>
                <input
                  type="number"
                  value={form.lucro_percentual}
                  onChange={(e) => setForm({ lucro_percentual: e.target.value })}
                  placeholder="Ex: 2.5"
                  step="0.1"
                  min="1"
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Valor mínimo recomendado: 1.5 — abaixo disso o produto pode ser vendido no prejuízo.
                </p>
              </div>
              ...
              <button
                ...
                disabled={salvando}
                className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 text-white text-sm font-medium rounded-lg py-2.5 transition-colors"
              >
          ...
          <div className="bg-gray-50 rounded-2xl border border-gray-200 p-6">
            <h3 className="text-base font-bold text-gray-800 mb-4">Exemplo com markup {markup || '?'}</h3>

            <div className="space-y-3">
              <div className="flex justify-between items-center py-2 border-b border-gray-200">
                <span className="text-sm text-gray-600">Custo total do produto</span>
                <span className="text-sm font-medium text-gray-800">R$ 100,00</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-gray-200">
                <span className="text-sm text-gray-600">Markup aplicado</span>
                <span className="text-sm font-medium text-gray-800">× {markup || '?'}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-gray-200">
                <span className="text-sm text-gray-600">Preço de venda</span>
                <span className="text-sm font-bold text-blue-600">
                  {exemploPrecoVenda !== '—' ? `R$ ${exemploPrecoVenda}` : '—'}
                </span>
              </div>
              <div className="flex justify-between items-center py-2">
                <span className="text-sm text-gray-600">Lucro</span>
                <span className="text-sm font-bold text-green-600">
                  {exemploLucro !== '—' ? `R$ ${exemploLucro} (${exemploPercentual}%)` : '—'}
                </span>
              </div>
            </div>
          </div>
